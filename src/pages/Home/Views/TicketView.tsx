...
const TicketView = ({reservationInfo}:any) => {
  ...
  return (
    <div style={{padding:'2rem 1rem',width:'100%',justifyContent:'center',alignItems:'center',display:'flex',position:'relative'}}>
        <img src={reservationInfo.detail.performanceDetailInfo.poster} alt="poster" style={{height:'20rem',width:'13rem',borderRadius:'1rem 0 0 1rem'}}/>
        ...
        <div style={{display:'grid',gridTemplateColumns: 'repeat(2, 1fr)',position:'absolute',top:'51%',left:'60%',transform:'translate(-50%,-50%)',justifyContent:'center',alignItems:'center'}} >
            <div style={{display:'grid',gridTemplateColumns: 'repeat(2, 1fr)',height:'20rem',width:'26rem',padding:'0'}}>
                ...
                <div style={{display:'flex',flexDirection:'column'}}>
                    <p>일시</p>
                    <p style={{fontSize:'1.4rem',fontWeight:'700'}}>{reservationInfo.selectedDate}</p>
                </div>

                <div style={{display:'flex',flexDirection:'column'}}>
                    <p>회차</p>
                    <p style={{fontSize:'1.4rem',fontWeight:'700'}}>{reservationInfo.selectedTime}</p>
                </div>

                <div style={{display:'flex',flexDirection:'column'}}>
                    <p>인원</p>
                    <p style={{fontSize:'1.4rem',fontWeight:'700'}}>{reservationInfo.people} 명</p>
                </div>

                <div style={{display:'flex',flexDirection:'column'}}>
                    <p>가격</p>
                    <p style={{fontSize:'1.4rem',fontWeight:'700'}}>{reservationInfo.seatPrice*reservationInfo.people}</p>
                </div>
            </div>
